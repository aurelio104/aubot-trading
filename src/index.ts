...
app.get("/loss-streak-recovery", async (_req, reply) => {
  const cached = loadRecoveryReport();
  if (cached) return cached;
  if (!hasCredentials()) {
    return reply.code(503).send({ error: "BINANCE credentials not configured" });
  }
  return runLossStreakRecovery(undefined, "manual");
});

app.get("/market-context", async (_req, reply) => {
  if (!hasCredentials()) {
    return reply.code(503).send({ error: "BINANCE credentials not configured" });
  }
  const btc = await buildBtcMarketContext();
  const regime =
    marketRegimeEnabled() ? getCachedRegime() ?? (await detectMarketRegime()) : null;
  const news = newsContextEnabled()
    ? (await ensureNewsContext()) ?? getCachedNews()
    : null;
  const free = await getFreeUsdt();
  const plan = buildAdaptiveCandidatePlan(free);

  return {
    at: new Date().toISOString(),
    unifiedGates: true,
    note: "Mismos gates con $21 o $1000 — el universo de pares crece con el capital",
    btc,
    candidatePlan: plan,
    regime,
    news: news
      ? {
          ...
          blockAllEntries: news.blockAllEntries,
          gateReason: news.gateReason,
          summaryEs: news.summaryEs,
        }
      : null,
  };
});
...
app.post<{
  ...
  if (action === "start") startEngine();
    ...
    const scorecard = computeScorecard(days);
    const learning = await runAutoApplyLearning(days);
    return { ...getStatus(), syncTrades: result, scorecard, learning };
  } else if (action === "clear_psychology_pause") {
    clearPsychologyPause();
    pushLog("info", "control clear_psychology_pause — pausa manual levantada");
    return {
      ...getStatus(),
      psychologyPause: false,
      message: "pausa psicológica levantada (solo si corregiste params y depositaste)",
    };
    ...
    return {
      ...
      blacklist: getExcludedSymbols(),
    };
  } else {
    return reply.code(400).send({
      error:
        "action: start | stop | pause | sync_position | refresh_analysis | set_symbol | redeem_earn | consolidate_usdt | analyze_rotation | rotate_if_better | set_strategy_params | set_blacklist | set_blocked_hours | refresh_news | sync_trades | clear_psychology_pause | apply_learning",
    });
  }
  return getStatus();
});
...
async function main(): Promise<void> {
  ...
  try {
    ...
    void runAutoApplyLearning(30).catch((e) => {
      pushLog("warn", `boot autoLearning: ${e instanceof Error ? e.message : String(e)}`);
    });
  } catch (e) {
    pushLog("warn", `boot pairWr: ${e instanceof Error ? e.message : String(e)}`);
  }
