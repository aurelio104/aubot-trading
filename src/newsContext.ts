...
export async function refreshNewsContext(): Promise<NewsSnapshot> {
  ...
  const fgExtremeBoost = Math.max(
    10,
    Number(process.env.AUBOT_FG_EXTREME_SCORE_BOOST || "15") || 15,
  );
  ...
  if (fg != null && fg.value <= fgBlockBelow() && !blockAll) {
    blockAll = true;
    gateReason = `Fear & Greed ${fg.value} ≤ ${fgBlockBelow()} — LONG bloqueado (Fase 1)`;
  }
