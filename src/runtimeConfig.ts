...
export function circuitBreakHours(): number {
  return Math.max(4, Number(process.env.AUBOT_CIRCUIT_BREAK_HOURS || "72") || 72);
}

let psychologyPauseUntil = 0;
let psychologyPauseReasonText = "";

export function isPsychologyPauseOpen(): boolean {
  if (Date.now() < psychologyPauseUntil) return true;
  if (psychologyPauseUntil > 0) {
    psychologyPauseUntil = 0;
    psychologyPauseReasonText = "";
  }
  return false;
}

export function psychologyPauseReason(): string {
  return psychologyPauseReasonText || "pausa psicológica activa";
}

export function psychologyPauseUntilIso(): string | null {
  if (!isPsychologyPauseOpen()) return null;
  return new Date(psychologyPauseUntil).toISOString();
}

/** @param extend false = no alargar pausa si ya hay una activa (evita resetear 48h cada tick) */
export function openPsychologyPause(
  hours: number,
  reason: string,
  extend = true,
): void {
  const ms = Math.max(0.05, hours) * 3600_000;
  const until = Date.now() + ms;
  if (!extend && isPsychologyPauseOpen()) return;
  if (extend && until > psychologyPauseUntil) {
    psychologyPauseUntil = until;
    psychologyPauseReasonText = reason;
  } else if (!extend && psychologyPauseUntil === 0) {
    psychologyPauseUntil = until;
    psychologyPauseReasonText = reason;
  }
}

export function clearPsychologyPause(): void {
  psychologyPauseUntil = 0;
  psychologyPauseReasonText = "";
}
