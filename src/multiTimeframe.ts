...
export async function checkMtfAlignment(symbol: string): Promise<MtfAlignment> {
  ...
  try {
    ...
    const h1Green =
      last.close >= last.open || (prev && last.close > prev.close);

    const maxH1 = Number(process.env.AUBOT_MTF_H1_RSI_MAX || "45") || 45;
    if (h1 > maxH1) {
      return {
        aligned: false,
        h1Rsi: h1,
    ...
    if (!h1Green && h1 > 38) {
      return {
        ...
        h1Rsi: h1,
        penalty: 12,
        reason: `1h bajista sin reversión (RSI ${h1.toFixed(1)})`,
      };
    }
    if (h1 > 42) {
      return {
