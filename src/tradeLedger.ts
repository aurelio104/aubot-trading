...
import { roundTripFeeUsdt } from "./capitalRules.js";
...
export async function recordSellClose(
  ...
  let buyFees = pending?.buyFeesUsdt ?? 0;
  if (buyFees <= 0 && quoteIn > 0) {
    buyFees = roundTripFeeUsdt(quoteIn) / 2;
    pushLog(
      "warn",
      `ledger: buy fees estimados ${buyFees.toFixed(4)} USDT (myTrades pendiente)`,
    );
  }
