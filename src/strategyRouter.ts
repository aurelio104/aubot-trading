...
export async function strategyConfig() {
  ...
  const bestRaw = analysis?.best ?? null;
  const best = maskOpportunityIfPaused(bestRaw, psych);
  return {
    ...
    analysis: analysis
      ? {
          ...
          best,
          bestRaw: psych.hideTemptationSignals ? bestRaw : undefined,
        }
      : null,
