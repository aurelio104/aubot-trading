...
export async function applyLearningFromScorecard(
  ...
  const tpFloor = Math.max(2.8, minGrossTpPct());

  if (n >= 2 && wr < 45) {
    adj.push(`WR bajo: más selectivo — RSI 28, TP ≥${tpFloor}%, capital 70%`);
    suggest.suggestedRsiBuyBelow = 28;
    suggest.suggestedTakeProfitPct = tpFloor;
    suggest.suggestedStopLossPct = Math.max(1.5, Number((tpFloor * 0.55).toFixed(2)));
    suggest.suggestedCapitalPct = 70;
  } else if (n >= 3 && wr >= 55) {
    adj.push("WR aceptable: mantener TP 3% SL 2%");
    suggest.suggestedTakeProfitPct = Math.max(3, tpFloor);
  }

  const feesDominate =
    scorecard.feesUsdt > Math.abs(scorecard.pnlUsdt) * 0.5 && n >= 2;
  if (feesDominate) {
    adj.push("Fees altos vs PnL: subir min net edge, TP mínimo fee-aware y menos rotaciones");
    suggest.suggestedMinNetEdgeUsdt = 0.22;
    suggest.suggestedTakeProfitPct = Math.max(
      Number(suggest.suggestedTakeProfitPct) || tpFloor,
      tpFloor,
    );
  }
