...
import { maybeRunNightlyPairWrUpdate } from "./pairWrStore.js";
import {
  consolidateEnabled,
  consolidateToUsdt,
...
import {
  isLossStreakPauseActive,
  maybeRunLossStreakRecoveryTick,
...
async function tick(): Promise<void> {
  ...
  try {
    ...
    if (lastPrice != null) {
      maybeRunNightlyPairWrUpdate();
      if (isLossStreakPauseActive() && tickCount % 30 === 0) {
        void maybeRunLossStreakRecoveryTick().catch(() => {});
      }
